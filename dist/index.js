...
var getScriptRoot, native_default;
var init_native = __esm({
  "src/native.ts"() {
    "use strict";
    getScriptRoot = () => process.cwd();
    native_default = Execute;
  }
});
...
__export(react_exports, {
  ...
  getValue: () => getValue2,
  hasValue: () => hasValue2,
  useConduit: () => useConduit
});
function extractJsonValue2(stdout) {
  const lines = stdout.trim().split("\n").reverse();
  for (const line of lines) {
    const trimmed = line.trim();
    if (!trimmed) continue;
...
async function Execute2(scriptPath, options = {}) {
  const apiEndpoint = options.apiEndpoint || "/api/conduit/execute";
  logDebug2(options.debug, "Executing", scriptPath, "via", apiEndpoint);
  try {
    ...
    const value = data.value !== void 0 ? data.value : extractJsonValue2(data.stdout || "");
    return {
    ...
    if (error.name === "AbortError") {
      logDebug2(options.debug, "Request aborted");
      return {
        status: "error",
        stdout: "",
    ...
    logDebug2(options.debug, "Execution error", error);
    return {
      status: "error",
      stdout: "",
...
function hasValue2(result, expected) {
  return result.value === expected;
}
function getValue2(result, path2, defaultValue) {
  if (!result.value || typeof result.value !== "object") {
    return defaultValue;
  }
  const keys = path2.split(".");
...
function useConduit() {
  ...
  if (!import_react.useState || !import_react.useCallback) {
    throw new Error("React hooks not found. Make sure React is loaded.");
  }
  const [loading, setLoading] = (0, import_react.useState)(false);
  const [error, setError] = (0, import_react.useState)(null);
  const [result, setResult] = (0, import_react.useState)(null);
  const execute = (0, import_react.useCallback)(async (scriptPath, options) => {
    setLoading(true);
    setError(null);
    try {
      const res = await Execute2(scriptPath, options);
      setResult(res);
      if (res.status === "error") {
        setError(res.stderr);
      }
...
var import_react, react_default;
var init_react = __esm({
  "src/react.ts"() {
    "use strict";
    import_react = __toESM(require_react());
    react_default = Execute2;
  }
});
...
if (isNode) {
  const { Execute: Execute3 } = (init_native(), __toCommonJS(native_exports));
  module.exports = Execute3;
  module.exports.default = Execute3;
} else {
  const { Execute: Execute3 } = (init_react(), __toCommonJS(react_exports));
  module.exports = Execute3;
  module.exports.default = Execute3;
}
export {
  Execute as ExecuteNative,
  Execute2 as ExecuteReact,
  RPL,
  RPL2 as RPLReact,
  getValue,
  getValue2 as getValueReact,
  hasValue,
  hasValue2 as hasValueReact,
  useConduit
};

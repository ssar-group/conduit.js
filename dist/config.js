...
const cache = new Map();
export function defineConduitConfig(config) {
    return config;
}
export async function loadConduitConfig(cwd = process.cwd(), options = {}) {
...
async function readConduitConfig(root, options) {
...
async function readJsonConfig(filePath) {
    try {
        ...
        throw new Error(formatError("Unable to read config", {
            file: filePath,
...
async function readJsConfig(filePath) {
    try {
        ...
        throw new Error(formatError("Unable to load config", {
            file: filePath,
            reason: error instanceof Error ? error.message : String(error),
        }));
    }
}
export function mergeConduitConfig(config, options) {
    const merged = {
        ...config,
        ...options,
        context: {
